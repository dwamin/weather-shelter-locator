...
export default function WeatherShelterPage() {
  ...
  const getStatusData = (t: number, p: PTYType) => {
    ...
    return { title: ptyData[p].title, iconUrl: ptyData[p].icon, desc };
  };
  ...
  return (
    <div className="max-w-md mx-auto bg-emerald-50 min-h-screen pb-24 font-sans text-gray-900 shadow-2xl relative">
        ...
        {userName ? (
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="text-xs px-3 py-1.5 bg-red-50 text-red-500 rounded-full border border-red-200 font-bold"
            >
              로그아웃
            </button>
          </div>
          ...
          <button
            ...
            className="text-xs px-4 py-2 bg-green-600 text-white rounded-full font-bold"
          >
            로그인
          </button>
        )}
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/90 backdrop-blur-md border-t border-emerald-100 flex justify-around py-3">
        ...
        {/* ✅ 로그인 탭: 로그인 전 → /login 이동 / 로그인 후 → 로그아웃 */}
        <NavItem
      ...
      {showDetail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/50 backdrop-blur-sm">
          <div className="bg-white w-full max-w-xs rounded-3xl p-6 shadow-2xl">
            ...
            <button onClick={() => setShowDetail(false)} className="mt-6 w-full py-3 bg-gray-900 text-white rounded-xl font-bold">
              닫기
            </button>
          </div>
        </div>
